...
export default function Feature({ navigation }) {
    ...
    return (
        <View style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', margin: height * 0.05, padding: height * 0.005,  }}>
            <View style={{ width: "100%", height: height / 2, justifyContent: 'center', alignItems: 'center' }}>
                <SliderBox style={{ width: 300, height: 300, marginLeft: 50 }} images={images} currentImageEmitter={i => setIndex(i)}
                    dotStyle={{ width: 10, height: 10, borderRadius: 50 }} dotColor="black" inactiveDotColor="green" autoPlay={true} autoPlayInterval={1} circleLoop={true} />
            </View>
            ...
            <AppButton onPress={() => navigation.navigate("Menu")} buttonText={'Next'} bgColor='#4a69bd' btnWidth={0.8} />
        </View>
    )
}